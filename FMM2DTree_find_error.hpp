...
//
//
#ifndef _FMM2DTree_find_error_HPP__
#define _FMM2DTree_find_error_HPP__
...
// public:
	...
	void getNodes() {
		for (int j=nLevels; j>=2; j--) {
			// double timegetNodes, start, end;
			// start	=	omp_get_wtime();
			getNodes_outgoing_level(j);
			// end		=	omp_get_wtime();
			// timegetNodes =	(end-start);
			// std::cout << std::endl << "j: " << j << "	Time taken getNodes_outgoing_level: " << timegetNodes << std::endl;

			// start	=	omp_get_wtime();
			getNodes_incoming_level(j);
			// end		=	omp_get_wtime();
			// timegetNodes =	(end-start);
			// std::cout << std::endl << "j: " << j << "	Time taken getNodes_incoming_level: " << timegetNodes << std::endl;

			// start	=	omp_get_wtime();
			get_L2P_P2M_level(j);
			// end		=	omp_get_wtime();
			// timegetNodes =	(end-start);
			// std::cout << std::endl << "j: " << j << "	Time taken get_L2P_P2M_level: " << timegetNodes << std::endl;
		}
	}
	...
	void getNodes_outgoing_level(int j) { //LFR; box interactions
		...
		for (int k=0; k<nBoxesPerLevel[j]; ++k) {
			...
			// std::cout << std::endl << "j: " << j << "	k: " << k << "	Rank: " << ComputedRank << "	Time taken getNodes_outgoing_box: " << timegetNodes << std::endl;
		}
	}
	...
	void getNodes_incoming_level(int j) { //LFR; box interactions
		...
		for (int k=0; k<nBoxesPerLevel[j]; ++k) {
			...
			// double end		=	omp_get_wtime();
			// double timegetNodes =	(end-start);
			// std::cout << std::endl << "j: " << j << "	k: " << k << "	Rank: " << ComputedRank << "	Time taken getNodes_incoming_box: " << timegetNodes << std::endl;
		}
	}
	...
	void getNodes_outgoing_box(int j, int k, int& n_rows, int& n_cols, int& ComputedRank) {
		...
		// 	tree[j][k].P2M = Mat(tree[j][k].outgoing_checkPoints.size(),tree[j][k].Ar.cols());
		// }
		// std::cout << "j: " << j << "	k: "<< k << "	P2M.N: " << tree[j][k].P2M.norm() << std::endl;
	}
...
///////////////////////////////////////////////////////////////////
  ...
  double error_check() {
		...
		int index = 0;
		for (size_t k = 0; k < nBoxesPerLevel[nLevels]; k++) {
			x.segment(index, tree[nLevels][k].particles.size()) = tree[nLevels][k].particles;
			index += tree[nLevels][k].particles.size();
		}
		index = 0;
		b_error_check = Vec::Zero(N);
		for (size_t k = 0; k < nBoxesPerLevel[nLevels]; k++) {
			b_error_check.segment(index, tree[nLevels][k].chargeLocations.size()) = tree[nLevels][k].particle_rhs;
			index += tree[nLevels][k].chargeLocations.size();
		}

    // string fname;
    // fname = "rhs_x/x_real_" + std::to_string((int)Qchoice) + "_" + std::to_string((int)N) + ".bin";
  	// Eigen::VectorXd x_real = storedata::load_vec(fname);
  	// fname = "rhs_x/x_imag_" + std::to_string((int)Qchoice) + "_" + std::to_string((int)N) + ".bin";
  	// Eigen::VectorXd x_imag = storedata::load_vec(fname);
    ...
    Vec x_true = x_real + I*x_imag;
		std::cout << "x_true.norm(): " << x_true.norm() << std::endl;
		Vec E = x-x_true;
		// std::cout << "x" << std::endl << x << std::endl;
		// std::cout << "x_true" << std::endl << x_true << std::endl;
		std::cout << "infinity norm of abs forward error: " << E.lpNorm<Infinity>() << std::endl;
		// std::cout << "forward rel error: " << E.norm()/x_true.norm() << std::endl;
		return E.norm()/x_true.norm();
	}
