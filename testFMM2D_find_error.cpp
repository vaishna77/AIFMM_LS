...
#include "FMM2DTree_find_error.hpp"
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "------------------------------------------------" << std::endl << std::endl;
	...
	// Eigen::VectorXd rhs_real = storedata::load_vec(fname);
	// fname = "rhs_x/rhs_imag_" + std::to_string((int)Qchoice) + "_" + std::to_string((int)N) + ".bin";
	// Eigen::VectorXd rhs_imag = storedata::load_vec(fname);
  ...
  std::cout << "rhs.norm(): " << rhs.norm() << std::endl;
	...
	for (unsigned j=0; j<N; ++j) {
			...
			for (unsigned k=0; k<n_Properties; ++k) {
					...
					properties[count_Property]      =       rhs(j);//10.0*double(rand())/double(RAND_MAX)-5.0 + I*(10.0*double(rand())/double(RAND_MAX)-5.0);
					++count_Property;
			}
	}
  //////////////////////////////// KD Tree //////////////////////////////////////////
